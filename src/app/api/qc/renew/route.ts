...
export async function POST(req: NextRequest) {
  ...
  try {
    const response = await fetch(`${base.replace(/\/$/, "")}/renew-session?market=${market}`, {
      method: "POST", headers: { Authorization: `Bearer ${token}` }, cache: "no-store", signal: AbortSignal.timeout(55000),
    });
